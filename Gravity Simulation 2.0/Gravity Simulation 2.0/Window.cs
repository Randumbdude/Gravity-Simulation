...
namespace Gravity_Simulation_2._0
{
    public partial class Window : Form
    {
        ...
        Body body2 = new Body(1, 10, 500, 350, -0.00004, -0.00004, new SolidBrush(Color.Cyan));
        Body body3 = new Body(1, 10, 550, 300, -0.00004, -0.00004, new SolidBrush(Color.Green));
        ...
        public Window()
        {
            ...
            this.simBox.MouseWheel += simBox_MouseWheel;
        ...
        private void simBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            e.Graphics.TranslateTransform(Constants.CameraX, Constants.CameraY);
            e.Graphics.ScaleTransform(Constants.zoomLevel, Constants.zoomLevel);
            ...
            try
            {
                linePoints.Add(new Point((int)(body2.X + body2.Radius), (int)(body2.Y + body2.Radius)));
                e.Graphics.DrawLines(Pens.White, linePoints.ToArray());
            }
            catch { }

            e.Graphics.ScaleTransform(1 / Constants.zoomLevel, 1 / Constants.zoomLevel);
            e.Graphics.TranslateTransform(-Constants.CameraX, -Constants.CameraY);

            e.Graphics.DrawString("frame rate: " + 1000 / Math.Max(_msFrameTime, 1), debugFont, debugBrush, new PointF(0, 0));
            e.Graphics.DrawString($"Camera: (X:{(int)Constants.CameraX},Y:{(int)Constants.CameraY})", debugFont, debugBrush, new PointF(0, 15));
            e.Graphics.DrawString($"Camera Zoom: {Constants.zoomLevel}", debugFont, debugBrush, new PointF(0, 30));

            //Console.WriteLine($"Body2 Velocity: (Vx: {body2.Vx}, Vy: {body2.Vy})");
        }
        ...
        private void simBox_MouseMove(object sender, MouseEventArgs e)
        {
            if (mouseDown && e.Button == MouseButtons.Right)
            {
                ...
                Constants.CameraX += (StartPointF.X - e.X) * -1 / 10;
                Constants.CameraY += (StartPointF.Y - e.Y) * -1 / 10;
            }
        }
        ...
        private void simBox_MouseWheel(object sender, MouseEventArgs e)
        {
            ...
            Constants.zoomLevel += zoomChange;

            // Limit the zoom level within a desired range
            Constants.zoomLevel = Math.Max(Constants.zoomLevel, 0.1f);
            Constants.zoomLevel = Math.Min(Constants.zoomLevel, 2.0f);
        }
