...
namespace Gravity_Simulation_2._0
{
    public partial class Window : Form
    {
        ...
        Body body2 = new Body(1000, 10, 500, 350, -0.00013, -0.00003, new SolidBrush(Color.Cyan));
        Body body3 = new Body(1000, 10, 550, 300, -0.00003, -0.00003, new SolidBrush(Color.Green));
        ...
        private void simBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            //linePoints.Add(new Point((int)body2.X, (int)body2.Y));

            //try { e.Graphics.DrawLines(Pens.Red, linePoints.ToArray()); } catch { }
        ...
        public void Simulate(double timeStep, double duration, double G)
        {
            ...
            for (int step = 0; step < steps; step++)
            {
                ...
                foreach (var body in bodies)
                {
                    ...
                    foreach (var otherBody in bodies)
                    {
                        if (body != otherBody)
                        {
                            ...
                            acceleration += (G * otherBody.Mass) / distSquared;
                        }
                    }
                ...
                foreach (var body in bodies)
                {
                    double newX = body.X + body.Vx * timeStep;
                    double newY = body.Y + body.Vy * timeStep;
                    double newVx = body.Vx;
                    double newVy = body.Vy;

                    foreach (var otherBody in bodies)
                    {
                        if (body != otherBody)
                        {
                            double dx = otherBody.X - newX;
                            double dy = otherBody.Y - newY;
                            double distSquared = dx * dx + dy * dy;
                            double force = (G * otherBody.Mass) / distSquared;
                            double accelerationX = force * dx;
                            double accelerationY = force * dy;

                            newVx += accelerationX * timeStep;
                            newVy += accelerationY * timeStep;
                        }
                    }
